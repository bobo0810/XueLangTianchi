...
class yoloDataset(data.Dataset):
    ...
    def __init__(self,root,list_file,train,transform):
        ...
        # xuelang_part2_3.txt每一行
        for line in lines:
            splited = line.strip().split()
            # 赋值图像名字
            self.fnames.append(splited[0]+' '+splited[1])
            # 赋值一张图的物体总数
            num_faces = int(splited[2])
            box=[]
            label=[]
            # 遍历一张图的所有物体
    ...
    def __getitem__(self,idx):
        ...
        # 读取图像
        img = cv2.imread(os.path.join(fname))
        # clone 深复制，不共享内存
        # 拿出对应的bbox及 标签对应的序号
        ...
        # 拿到图像对应的真值，以便计算loss
        target = self.encoder(boxes,labels)# 7x7x58    一张图被分为7x7的网格;58=（2x5+48） 一个网格预测两个框   一个网格预测所有分类概率，数据集分类为43类
        # 图像转化
    ...
    def encoder(self,boxes,labels):
        ...
        for i in range(cxcy.size()[0]):
            ...
            # 加9是因为前0-9为两个真实候选框的值。后10-20为20分类   将对应分类标为1
